...
// significant figures in the given number. You can read about significant figures below.
...
function numberOfSigfigs(n) {
    ...
    for(i = 0; i < n.length; i++){
        if (m[0] === '0') {m.shift();} else { break; }
    }
    if(!m.includes('.')){
        for (i = 0; i < n.length; i++){
        if (m[m.length - 1] === '0' ) {m.pop();} else { break; }
        }
    }
    return m.filter(e => e !== '.').length;
  }
